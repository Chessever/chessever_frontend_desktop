...
void main() {
  ...
  group('ChessboardController tree reattachment', () {
    ...
    test('detached animation getters fall back instead of null-crashing', () {
      ...
      // transient detach/reattach window. In that state the controller's curved
      // animations are intentionally null; the getters must not throw a null-
      // check crash when painters ask for them.
      controller.detach();

      expect(controller.fadeAnimation.value, 1.0);
      expect(controller.translationAnimation.value, 1.0);
    });
