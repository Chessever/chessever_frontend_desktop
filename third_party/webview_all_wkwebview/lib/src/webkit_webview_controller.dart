...
class WebKitWebViewControllerCreationParams
  ...
  WebKitWebViewControllerCreationParams({
    ...
    this.elementFullscreenEnabled = false,
  }) {
  ...
  WebKitWebViewControllerCreationParams.fromPlatformWebViewControllerCreationParams(
    ...
    bool elementFullscreenEnabled = false,
  }) : this(
  ...
  /// Whether page content may enter full screen through the HTML Fullscreen
  /// API (`Element.requestFullscreen`, and a video player's own fullscreen
  /// control).
  ///
  /// WKWebView ships with this off, which hides or disables the fullscreen
  /// control of embedded players. On macOS, WebKit presents the element in a
  /// full-screen window of its own. Requires iOS 15.4 / macOS 12.3; ignored
  /// on older systems.
  ///
  /// Defaults to false.
  final bool elementFullscreenEnabled;
}
...
class WebKitWebViewController extends PlatformWebViewController {
  ...
  WebKitWebViewController(PlatformWebViewControllerCreationParams params)
    ...
    if (_webKitParams.elementFullscreenEnabled) {
      unawaited(_enableElementFullscreen());
    }
  }

  /// WKWebView copies its configuration at creation, but the copy shares the
  /// same `WKPreferences` instance and WebKit pushes preference changes to
  /// the page, so the switch takes effect for every later load.
  Future<void> _enableElementFullscreen() async {
    try {
      final WKPreferences preferences = await _webView.configuration
          .getPreferences();
      await preferences.setElementFullscreenEnabled(true);
    } catch (error) {
      debugPrint(
        'webview_all_wkwebview: failed to enable element fullscreen; page '
        'content cannot enter full screen: '
        '${error.toString().replaceAll(RegExp(r'[\r\n]+'), ' ')}',
      );
    }
  }
