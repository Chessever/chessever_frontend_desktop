...
final class PigeonApiWKPreferences: PigeonApiProtocolWKPreferences {
  ...
  static func setUpMessageHandlers(
    ...
    let setElementFullscreenEnabledChannel = FlutterBasicMessageChannel(
      name:
        "com.abandoft.pigeon.webview_all_wkwebview.WKPreferences.setElementFullscreenEnabled",
      binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      setElementFullscreenEnabledChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let pigeonInstanceArg = args[0] as! WKPreferences
        let enabledArg = args[1] as! Bool
        do {
          try api.pigeonDelegate.setElementFullscreenEnabled(
            pigeonApi: api, pigeonInstance: pigeonInstanceArg, enabled: enabledArg)
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
      ...
      setElementFullscreenEnabledChannel.setMessageHandler(nil)
    }
  }
