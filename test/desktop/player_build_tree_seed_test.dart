...
void main() {
  test(
    'player tree controller builds from the optimized backend snapshot',
    () async {
  ...
  test('retry reuses the backend tree without forcing a rebuild', () async {
    final repository = _BackendTreeRepository();
    final container = ProviderContainer(
      overrides: [gamebaseRepositoryProvider.overrideWithValue(repository)],
    );
  ...
  test('accepts a ready snapshot before status changes to ready', () async {
  ...
  test('downloads example games separately and only once', () async {
    final repository = _BackendTreeRepository();
    final container = ProviderContainer(
      overrides: [gamebaseRepositoryProvider.overrideWithValue(repository)],
    );
