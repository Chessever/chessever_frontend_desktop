...
void main() {
  ...
  group('getGameByAnyId — UUID routing', () {
    ...
    test(
      'uppercase UUID routes to getGameById (case-insensitive regex)',
      () async {
        const id = 'FE6351A5-6354-4C16-B7F6-9124E5D9A9EF';
        await repo.getGameByAnyId(id);
    ...
    test(
      'UUID with leading/trailing whitespace is trimmed then routes to getGameById',
      () async {
        const bare = 'fe6351a5-6354-4c16-b7f6-9124e5d9a9ef';
        await repo.getGameByAnyId('  $bare  ');

        expect(
          repo.calledGetByIdWith,
          equals(bare),
          reason: 'trimmed value passed',
        );
        expect(repo.calledGetByLichessIdWith, isNull);
      },
    );
  ...
  group('getGameByAnyId — namespaced canonical ID routing', () {
    test('direct-broadcast ID queries games.id', () async {
      const id = 'chess.com:14987654321';
      await repo.getGameByAnyId(id);
    ...
    test('multi-part canonical ID remains intact', () async {
      const id = 'chess-results:event:round:board-7';
      await repo.getGameByAnyId('  $id  ');
  ...
  group('getGameByAnyId — short-ID (Lichess) routing', () {
    test(
      '8-char alphanumeric Lichess ID routes to getGameByLichessId',
      () async {
        const id = '4uVwSr9q';
        await repo.getGameByAnyId(id);
    ...
    test(
      'short ID with leading/trailing whitespace is trimmed then routes to getGameByLichessId',
      () async {
        const bare = '4uVwSr9q';
        await repo.getGameByAnyId('  $bare  ');

        expect(
          repo.calledGetByLichessIdWith,
          equals(bare),
          reason: 'trimmed value passed',
        );
        expect(repo.calledGetByIdWith, isNull);
  ...
  // -------------------------------------------------------------------------
  group(
    'getGameByAnyId — non-UUID formats always route to getGameByLichessId',
    () {
  ...
  group('getGameByAnyId — no cross-contamination between sequential calls', () {
    test(
      'Lichess call followed by UUID call: each uses the correct method',
      () async {
        ...
        expect(
          repo.calledGetByIdWith,
          equals('fe6351a5-6354-4c16-b7f6-9124e5d9a9ef'),
        );
        expect(repo.calledGetByLichessIdWith, isNull);
      },
    );
    ...
    test(
      'UUID call followed by Lichess call: each uses the correct method',
      () async {
