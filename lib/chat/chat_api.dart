...
/// Parses one NDJSON line from the message stream.
///
/// Blank lines and lines that are not a JSON object (for example a final line
/// cut off when the connection drops) yield null instead of throwing, so a
/// torn stream ends cleanly and the caller reconciles against persisted
/// history.
ChatStreamEvent? parseChatStreamLine(String line) {
  ...
  return ChatStreamEvent(decoded['type'] as String? ?? 'error', decoded);
}

Stream<ChatStreamEvent> decodeChatStreamLines(Stream<String> lines) async* {
  await for (final line in lines) {
    final event = parseChatStreamLine(line);
