...
11. **Exactly one release per platform lives on the release server.** `KEEP_LAST_N=1` in `scripts/codemagic_publish_wrapper.sh` is the single retention knob; it is passed to every `codemagic-finalize prepare|ingest|clear-legacy|prune-downloads` call. One release per platform means one `archive/<version+build-platform>/` directory, one `app-archive.json` item, and one versioned download per platform — plus the stable aliases (`Chessever.dmg`, `Chessever-intel.dmg`, `Chessever-Setup.exe`, `Chessever.deb`), which are overwritten in place and never counted as history.
